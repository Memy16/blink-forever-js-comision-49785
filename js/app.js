        ...
        function mostrarResultadoFinal() {
            const resultadoFinalDiv = document.createElement('div');
            resultadoFinalDiv.style.position = 'fixed';
            resultadoFinalDiv.style.top = '50%';
            resultadoFinalDiv.style.left = '50%';
            resultadoFinalDiv.style.transform = 'translate(-50%, -50%)';
            resultadoFinalDiv.style.padding = '20px';
            resultadoFinalDiv.style.borderRadius = '10px';
            resultadoFinalDiv.style.background = '#333';
            resultadoFinalDiv.style.color = '#fff';
            resultadoFinalDiv.style.textAlign = 'center';
            resultadoFinalDiv.style.zIndex = '9999';
        
            const mensajeFinal = document.createElement('p');
            mensajeFinal.textContent = `Juego terminado. Puntuación final: ${puntos}/${preguntas.length}`;
            const botonReiniciar = document.createElement('button');
            botonReiniciar.textContent = 'Reiniciar Juego';
            botonReiniciar.addEventListener('click', reiniciarJuego);
        
            resultadoFinalDiv.appendChild(mensajeFinal);
            resultadoFinalDiv.appendChild(botonReiniciar);
            document.body.appendChild(resultadoFinalDiv);
        }
        ...
        function reiniciarJuego() {
            ...
            puntos = 0;
            mostrarPregunta();
            document.body.removeChild(document.querySelector('#resultadoFinal'));
        }
        function siguientePregunta() {
            preguntaActual++;
